...
def get_tags_by_plot_id(request, instance, plot_id):
    ...
    tag_list = context_dict_for_tags_list(request,
                                          TaggedItem.objects.filter(object_id=tree_dict['id']))  # noqa: E501
    return tag_list
...
def remove_tag(request, instance, plot_id, tag):
    tree = Tree.objects.get(plot_id=plot_id)
    tree_dict = context_dict_for_tree(request, tree)
    tag_list = context_dict_for_tags_list(request, TaggedItem.objects.filter(object_id=tree_dict['id']))  # noqa: E501
