...
class GrowthModelUrbanTreeDatabase(object):
    ...
    def init_tree(self, tree):
        ...
        if tree.diameter < min_diameter:
            ...
            tree.initial_age = min_age - \
                (min_diameter - tree.diameter) / growth_rate_at_min_age
...
def _get_species_for_planting(instance):
    ...
    def can_get_species(otm_code):
        return Species.get_by_code(
            instance, otm_code, itree_region.code) is not None
...
_growth_data = {
    'CaNCCoJBK': {  # NoCalC
        ...
        # max_age lowered
        'ROPS': (cub(2.84813, 0.18509, 0.07077, -0.00096), 2.85, 50.85),
        'SESE': (lin(2.79785, 1.99742), 2.8, 154.6),
        'ULAM': (lin(2.3541, 1.40641), 2.35, 121.9),
        # max_age lowered
        'ULPA': (cub(2.85154, -0.15554, 0.07672, -0.00112), 2.84, 44.84),
    },
    'CenFlaXXX': {  # CenFla
        # max_age lowered
        'ACRU': (quad(-3.6811, 3.0548, -0.02326), 2.34, 65.34),
        # max_age lowered
        'CICA': (quad(-0.87708, 2.85059, -0.01321), 1.96, 107.96),
        'ERJA': (lin(1.72609, 1.77157), 1.73, 54.87),
        # max_age lowered
        'JUSI': (cub(-0.6109, 1.63311, 0.0145, -0.00022), 1.04, 76.04),
        # max_age lowered
        'KOEL': (quad(1.54502, 2.71251, -0.02975), 1.55, 45.55),
        'LAIN': (lin(5.70526, 1.95628), 5.71, 64.39),
        'LIST': (loglogw1(0.76105, 2.6362, 0.03839), 0.88, 99),
        # max_age lowered
        'MAGR': (quad(-1.74323, 2.41888, -0.01267), 3.04, 95.04),
        'PIEL': (loglogw1(0.23241, 3.06738, 0.12536), 2, 88.28),
        'PLOC': (lin(-1.10175, 1.91487), 2.73, 94.64),
        # max_age lowered
        'PRCA': (quad(-2.76632, 3.70913, -0.04597), 0.9, 39.9),
        # max_age lowered
        'QULA2': (cub(1.30621, 1.23988, 0.04912, -0.00065), 1.31, 61.31),
    ...
    'GulfCoCHS': {  # GulfCo
        ...
        # max_age lowered
        'QUNI': (cub(2.53058, 0.94546, 0.0262, -0.00032), 2.53, 68.53),
        'QUPH': (lin(2.52535, 1.46083), 2.53, 115.01),
        # max_age lowered
        'QUVI': (quad(0.94464, 2.24738, -0.01091), 0.94, 102.94),
    },
    'InlEmpCLM': {  # InlEmp
    ...
    'InlValMOD': {  # InlVal
        ...
        # max_age lowered
        'CESI4': (quad(2.5184, 2.16011, -0.01839), 2.52, 58.52),
        'CICA': (quad(2.52476, 1.52318, -0.00624), 2.52, 95.47),
        # max_age lowered
        'FRAN_R': (quad(2.51452, 2.54844, -0.03194), 2.51, 39.51),
        'FREXH': (lin(3.06926, 1.12052), 3.07, 70.3),
        # max_age lowered
        'FRHO': (quad(2.51709, 2.29152, -0.01509), 2.52, 75.52),
        'FRPEM': (quad(2.52274, 1.72537, -0.01816), 2.52, 43.49),
        'FRVEG': (quad(2.52059, 1.94085, -0.01297), 2.52, 75.13),
        # max_age lowered
        'GIBI': (cub(2.54104, -0.10418, 0.0531, -0.00052), 2.49, 67.49),
    ...
    'InterWABQ': {  # InterW
        ...
        # max_age lowered
        'FRVE': (cub(4.61345, 0.39975, 0.05339, -0.00074), 4.61, 51.61),
        ...
        # max_age lowered
        'POAN': (quad(0.30296, 2.68255, -0.0176), 2.97, 75.97),
    ...
    'LoMidWXXX': {  # LoMidW
        # max_age lowered
        'ACPL': (cub(0.98208, 0.25006, 0.03658, -0.0003), 0.98, 84.98),
        ...
        # max_age lowered
        'PODE': (cub(-23.6314, 6.13541, -0.13834, 0.00102), 3.71, 38.71),
    ...
    'MidWstMSP': {  # MidWst
        ...
        # max_age lowered
        'ACSA1': (cub(-1.7914, 1.88949, 0.06576, -0.00121), 2.24, 47.24),
        # max_age lowered
        'ACSA2': (cub(2.52368, 1.31314, 0.01822, -0.00028), 2.52, 66.52),
        # max_age lowered
        'CEOC': (cub(3.54797, 1.18435, 0.0522, -0.00081), 3.55, 52.55),
        # max_age lowered
        'FRAM': (cub(2.83778, 0.75231, 0.15946, -0.00486), 2.84, 23.84),
        # max_age lowered
        'FRPE': (quad(-2.71787, 2.37818, -0.01186), 1.99, 99.99),
        'GIBI': (lin(3.58555, 1.14093), 3.59, 49.22),
        # max_age lowered
        'GLTR': (cub(4.22795, -0.82484, 0.37243, -0.01051), 3.77, 22.77),
        'MA2': (cub(3.23454, 1.60114, -0.0723, 0.00185), 3.23, 69.76),
        # max_age lowered
        'QUPA': (cub(3.31385, -0.16442, 0.14156, -0.00245), 3.29, 38.29),
        'QURU': (quad(1.99374, 1.63948, -0.00829), 1.99, 68.49),
        'TIAM': (cub(3.29265, 1.6335, -0.02574, 0.0004), 3.29, 117.21),
        # max_age lowered
        'TICO': (cub(3.31492, 1.20157, 0.13063, -0.00336), 3.31, 30.31),
    ...
    'NMtnPrFNL': {  # NMtnPr
        ...
        # max_age lowered
        'FRAM': (cub(2.8422, 0.77951, 0.03184, -0.00039), 2.84, 64.84),
        ...
        'MA2': (lin(2.48841, 1.15593), 2.49, 60.28),
        # max_age lowered
        'PINI': (cub(3.16683, 0.6726, 0.06006, -0.00121), 3.17, 38.17),
    ...
    'PacfNWLOG': {  # PacfNW
        ...
        # max_age lowered
        'ACSA2': (cub(1.47953, 1.65885, 0.00513, -0.00015), 1.48, 73.48),
        # max_age lowered
        'BEPE': (quad(-0.91466, 2.11608, -0.02273), 1.18, 46.18),
        ...
        # max_age lowered
        'PSME': (quad(-1.19688, 2.36185, -0.01267), 1.15, 93.15),
        'PYAN': (quad(0.78363989, 0.885844908, -0.009487773), 0.78, 21.46),
        # max_age lowered
        'QURU': (cub(0.12978, 1.83846, 0.02001, -0.00036), 1.99, 63.99),
        # max_age lowered
        'TIAM': (cub(-0.57558, 1.6899, 0.00442, -0.00016), 1.12, 69.12),
    ...
    'PiedmtCLT': {  # Piedmt
        ...
        # max_age lowered
        'ACSA2': (cub(-1.68001, 2.32536, -0.00094, -0.00023), 0.64, 56.64),
        ...
        # max_age lowered
        'LA6': (cub(2.53768, 0.23253, 0.10404, -0.002), 2.54, 35.54),
        ...
        # max_age lowered
        'QUNI': (cub(4.51977, 1.26018, 0.02646, -0.00032), 4.52, 73.52),
    ...
    'SacVal': {  # Not an i-Tree region??
        ...
        # max_age lowered
        'QUAG': (cub(2.25596, 1.43382, 0.01147, -0.00018), 2.26, 77.26),
        # max_age lowered
        'QULO': (cub(2.28211, 1.66463, -0.01025, 0.00002), 2.28, 133.28),
    ...
    'SWDsrtGDL': {  # SWDsrt
        ...
        # max_age lowered
        'MOAL': (cub(0.58767, 1.3122, 0.00991, -0.00014), 1.91, 83.91),
    ...
    'TpIntWBOI': {  # TpIntW
        ...
        # max_age lowered
        'ACSA1': (cub(4.03735, 0.99779, 0.01328, -0.00011), 4.04, 108.04),
        ...
        # max_age lowered
        'ROPS': (cub(2.53091, 0.90726, 0.02391, -0.00026), 2.53, 76.53),
        'TIAM': (quad(0.17199, 1.56439, -0.00867), 3.27, 70.72),
        # max_age lowered
        'ULPU': (cub(-2.208, 1.87431, 0.00437, -0.00016), 1.56, 72.56),
    },
    'TropicPacXXX': {  # Tropic
